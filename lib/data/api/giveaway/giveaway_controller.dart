...
class GiveawayController extends GetxController{
  ...
  Future<Map<String, dynamic>> getGiveawayById(int giveawayId) async {
    accessToken = await AppConstrants.getToken();
    var headers = {
      'Content-Type': 'application/json',
      'Authorization': 'Bearer $accessToken'
    };
    var request = http.Request('GET', Uri.parse(AppConstrants.GETGIVEAWAYBYID_URL +giveawayId.toString()));
    ...
    if (response.statusCode == 200) {
      ...
      update();
      print("getGiveawayListById " + resultList.toString());
      return resultList;
    } else {
      print(response.statusCode);
      print(response.reasonPhrase);
